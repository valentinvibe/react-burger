...
const OrderCard = ({ order, viewStatus }) => {
  ...
  return (
    <li className={`${styles.wrapper}`}>
      ...
      {viewStatus && <p className="text text_type_main-default"> 
        {order.status === "done"
        ? "Выполнен"
      ...
      <div className={styles.orderInfo}>
        ...
        {arrIngredientsLength <= 5 && orderIngredientsData.map((item, index) => {
                  ...
                  {item && (
                    <>
                      <div className={styles.hidePic}>
                        <IngredientImage image={item.image} alt={item.name} hide="true"/>
                        <p
